...
```{r densities_per_event, out.width="100%", warning=FALSE, message=FALSE}
...
event_names <- c("MmuINT0172005", "MmuINT1033613")
...
# Render DataTable with enhancements
datatable(
  select(final_exon_list, -ENTREZID,-sequence, -avg_Pdiff, -avg_FDR),
  options = list(
    pageLength = 10,                      # Rows per page
    autoWidth = TRUE,                     # Adjust column widths automatically
    dom = 'Bfrtip',                       # Add buttons for export
...
# Render DataTable with enhancements
datatable(
  select(final_intron_list, -ENTREZID,-sequence, -avg_Pdiff, -avg_FDR),
  options = list(
    pageLength = 10,                      # Rows per page
    autoWidth = TRUE,                     # Adjust column widths automatically
    dom = 'Bfrtip',                       # Add buttons for export
...
```{r }
# Generate individual plots for every unique event in the EVENT column
unique_event_names <- unique(tao_anova_multgroups_exons_tab$EVENT)  # Get unique events

# Loop over each event and generate a plot
for (eventID in unique_event_names) {
  # Extract the corresponding gene name for the event
  geneName <- final_exon_list$GENE[final_exon_list$EVENT == eventID]
  ...
  ggsave(
    filename = paste0("plot_", eventID, ".png"),  # File name
    plot = plot,
