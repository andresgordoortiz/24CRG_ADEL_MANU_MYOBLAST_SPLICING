...
install_load("ggtext")
install_load("readxl")
install_load("biomaRt")
library("patchwork")
library("org.Mm.eg.db")
install_load("org.Hs.eg.db")
library("enrichplot")
library("DOSE")
...
plot_absolute <- ggplot(splicing_events_long_sum, aes(y = count, x = study)) + 
  ...
  labs(
    x = "Study",
    y = "Number of Events",
    caption = "Data source: Splicing Event Dataset"
  ) +
  theme_minimal(base_family = font) +
  theme(
...
go_enrich_exon <- enrichGO(gene = exon_go$ENTREZID,
                      ...
                      pvalueCutoff = 0.05,
                      qvalueCutoff = 0.05)
speckle_related_genes<-strsplit(head(go_enrich_exon)[1, "geneID"], "/")[[1]]
...
go_enrich_intron <- enrichGO(gene = intron_go$ENTREZID,
                      ...
                      pvalueCutoff = 0.05,
                      qvalueCutoff = 0.05)
speckle_related_genes<-strsplit(head(go_enrich_intron)[5, "geneID"], "/")[[1]]
...
dotplot(gene_entrez_go, showCategory = 20) + ggtitle("GO Enrichment Analysis")
```
...
deltapsi_coef<- 5000
fstat_coef<-0.8
gc_coef <- 1.2 # Weight for GC content
length_coef <- -0.0005 # Weight for length (inverse relation)
splice_site_coef <- -0.8 # Weight for splice site score (inverse relation)
go_bonus <- 3 # Bonus for Gene ontology related
experiment_bonus_A <- 5.5 # Bonus for found experimentally
experiment_bonus_B <- 2 # Bonus for found experimentally

# Linear model calculation Introns
final_intron_list$Final_Score <- (gc_coef * final_intron_list$GC_percent) +
  (length_coef * (final_intron_list$length)) +
  (splice_site_coef * final_intron_list$splice_score_3) +
  (splice_site_coef * final_intron_list$splice_score_5) +
  (ifelse(final_intron_list$GO_speckle, go_bonus, 0)) +
  (ifelse(final_intron_list$GroupA, experiment_bonus_A, 0)) +
  (ifelse(final_intron_list$GroupB, experiment_bonus_B, 0)) +
  (deltapsi_coef * abs(final_intron_list$avg_deltapsi)) +
  (fstat_coef * final_intron_list$avg_Fstat) 

# Linear model calculation Exons
final_exon_list$Final_Score <- (gc_coef * final_exon_list$GC_percent) +
  (length_coef * (final_exon_list$length)) +
  (splice_site_coef * final_exon_list$splice_score_3) +
  (splice_site_coef * final_exon_list$splice_score_5) +
  (ifelse(final_exon_list$GO_speckle, go_bonus, 0)) +
  (ifelse(final_exon_list$GroupA, experiment_bonus_A, 0)) +
  (ifelse(final_exon_list$GroupB, experiment_bonus_B, 0)) +
  (deltapsi_coef * abs(final_exon_list$avg_deltapsi)) +
  (fstat_coef * final_exon_list$avg_Fstat)

final_exon_list <- final_exon_list[!duplicated(final_exon_list$EVENT), ] %>%
  arrange(desc(Final_Score))
final_intron_list<-arrange(final_intron_list, desc(Final_Score))
```
...
final_figure_exons <- wrap_plots(plots, ncol = 3, byrow = TRUE) + 
...
```{r densities_per_event introns, fig.width=14, fig.height=10, out.width="100%", warning=FALSE, message=FALSE}
# Adjust your plotting function
plots <- lapply(1:6, function(i) {
  eventID <- final_intron_list$EVENT[i]
  geneName <- final_intron_list$GENE[i]
...
# Adjust the number of columns for wrapping and add the Y-axis once per row
final_figure_introns <- wrap_plots(plots, ncol = 3, byrow = TRUE) + 
  plot_layout(guides = 'collect') + 
  theme(plot.margin = margin(0, 0, 0, 0))  # Adjust margins for consistent spacing

# Display the combined figure
final_figure_introns
