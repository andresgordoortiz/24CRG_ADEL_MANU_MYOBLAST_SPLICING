...
jid3=$(sbatch --dependency=afterok:$jid1 $PWD/scripts/bash/trapnell/vast_align_trapnell.sh | tr -cd '[:digit:].')
echo "...third job ID is $jid3"

# Fourth job - vast combine
echo "Submitting fourth job: vast combine..."
jid4=$(sbatch --dependency=afterok:$jid3 $PWD/scripts/bash/trapnell/vast_combine_trapnell.sh | tr -cd '[:digit:].')
