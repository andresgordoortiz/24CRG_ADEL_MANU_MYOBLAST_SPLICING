...
jid3=$(sbatch --dependency=afterok:$jid1 $PWD/scripts/bash/lyu_cells2022/vast_align_lyu.sh $VASTDB_PATH | tr -cd '[:digit:].')
echo "...second job ID is $jid3"

# Fourth job - run vast combine (dependent on second job)
echo "Submitting third job: Run vast combine..."
jid4=$(sbatch --dependency=afterok:$jid3 $PWD/scripts/bash/lyu_cells2022/vast_combine_lyu.sh $VASTDB_PATH | tr -cd '[:digit:].')
