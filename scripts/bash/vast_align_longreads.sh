...
files=($PWD/data/processed/longreads_elisabeth/*.fastq.gz)
file=${files[$SLURM_ARRAY_TASK_ID]}

basename=$(basename "$file" .fastq.gz)
mkdir -p $PWD/data/processed/longreads_elisabeth/vast_out
...
/users/mirimia/projects/vast-tools/vast-tools align \
    "$file" \
    -sp mm10 \
    -o $PWD/data/processed/longreads_elisabeth/vast_out \
    --IR_version 2 \
    -c 8 \
    -n "$basename"
